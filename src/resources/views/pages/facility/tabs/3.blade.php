...
@section('graphs')
    ...
    @elseif ($current_facility->id == '6dc0272e-be4e-4d94-bccd-7f6f3b78289c')
        <div class="graph" id="container1" data-title="Composition biométhane (GrDF)" data-keys="COMPO_CH4_GRDF,COMPO_CO2_GRDF,COMPO_H2_GRDF,COMPO_H2S_GRDF,COMPO_N2_GRDF,COMPO_O2_GRDF" data-legend="CH<sub>4</sub> (%),CO<sub>2</sub> (%),H<sub>2</sub>O (%HR),H<sub>2</sub>S (ppm), N<sub>2</sub> (%), O<sub>2</sub> (%)"></div>
        @include('biomet::pages.facility.includes.excel_button')
    @endif
@endsection
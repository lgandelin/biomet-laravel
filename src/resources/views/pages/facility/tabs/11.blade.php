        ...
        <div class="box-content facility-template">
            ...
            <div style="margin-bottom: 3rem;">
                ...
                <form action="{{ route('facility_group_excel') }}" method="post">
                    <div style="display:inline-block"><strong>Date de début :</strong> <input id="start_date" name="start_date" class="datepicker form-control" value="@if (isset($start_date)){{$start_date}}@else{{ date('d/m/Y', strtotime( '-1 days' )) }}@endif" style="display: inline; width:175px; margin-bottom: 1rem; margin-right: 2.5rem;"/></div>
                    <div style="display:inline-block"><strong>Date de fin :</strong> <input id="end_date" name="end_date" class="datepicker form-control" value="@if (isset($end_date)){{$end_date}}@else{{ date('d/m/Y', strtotime( '-1 days' )) }}@endif" style="display: inline; width:175px; margin-bottom: 1rem; margin-right: 2.5rem;"/></div>
                    <input type="submit" class="btn btn-valid" value="{{ trans('biomet::generic.valid') }}" />
                    {{ csrf_field() }}
